...
#   com.telemetria.integration.nfe.schemas         (tipos nao-evento)
#   com.telemetria.integration.nfe.schemas_eventos (todos os eventos)
#
...
for file in "${FILES[@]}"; do
    ...
    modified=$(echo "$modified" | sed \
        -e 's/br\.com\.swconsultoria\.nfe\.schema_4\.[a-zA-Z0-9_]*\./com.telemetria.integration.nfe.schemas./g')

    # schema.consCad, schema.distDFe, schema.resNFe, schema.comum → schemas
    modified=$(echo "$modified" | sed \
        -e 's/br\.com\.swconsultoria\.nfe\.schema\.consCad\./com.telemetria.integration.nfe.schemas./g' \
        -e 's/br\.com\.swconsultoria\.nfe\.schema\.distDFe\./com.telemetria.integration.nfe.schemas./g' \
        -e 's/br\.com\.swconsultoria\.nfe\.schema\.resNFe\./com.telemetria.integration.nfe.schemas./g' \
        -e 's/br\.com\.swconsultoria\.nfe\.schema\.comum\./com.telemetria.integration.nfe.schemas./g')

    # schema.envcce / schema.cce → schemas_eventos
    modified=$(echo "$modified" | sed \
        -e 's/br\.com\.swconsultoria\.nfe\.schema\.envcce\./com.telemetria.integration.nfe.schemas_eventos./g' \
        -e 's/br\.com\.swconsultoria\.nfe\.schema\.cce\./com.telemetria.integration.nfe.schemas_eventos./g')

    # schema.envEventoCancNFe, schema.eventoCancNFe → schemas_eventos
    modified=$(echo "$modified" | sed \
        -e 's/br\.com\.swconsultoria\.nfe\.schema\.envEventoCancNFe\./com.telemetria.integration.nfe.schemas_eventos./g' \
        -e 's/br\.com\.swconsultoria\.nfe\.schema\.eventoCancNFe\./com.telemetria.integration.nfe.schemas_eventos./g')

    # schema.envEventoCancSubst → schemas_eventos
    modified=$(echo "$modified" | sed \
        -e 's/br\.com\.swconsultoria\.nfe\.schema\.envEventoCancSubst\./com.telemetria.integration.nfe.schemas_eventos./g')

    # schema.envConfRecebto, schema.confRecebto → schemas_eventos (manifestacao)
    modified=$(echo "$modified" | sed \
        -e 's/br\.com\.swconsultoria\.nfe\.schema\.envConfRecebto\./com.telemetria.integration.nfe.schemas_eventos./g' \
        -e 's/br\.com\.swconsultoria\.nfe\.schema\.confRecebto\./com.telemetria.integration.nfe.schemas_eventos./g')

    # schema.envEpec, schema.epec → schemas_eventos
    modified=$(echo "$modified" | sed \
        -e 's/br\.com\.swconsultoria\.nfe\.schema\.envEpec\./com.telemetria.integration.nfe.schemas_eventos./g' \
        -e 's/br\.com\.swconsultoria\.nfe\.schema\.epec\./com.telemetria.integration.nfe.schemas_eventos./g')

    # schema.envEventoAtorInteressado → schemas_eventos
    modified=$(echo "$modified" | sed \
        -e 's/br\.com\.swconsultoria\.nfe\.schema\.envEventoAtorInteressado\./com.telemetria.integration.nfe.schemas_eventos./g')

    # schema.eventoGenerico → schemas_eventos
    modified=$(echo "$modified" | sed \
        -e 's/br\.com\.swconsultoria\.nfe\.schema\.eventoGenerico\./com.telemetria.integration.nfe.schemas_eventos./g')

    # schema.envEventoEConf, schema.envEventoCancEConf → schemas_eventos
    modified=$(echo "$modified" | sed \
        -e 's/br\.com\.swconsultoria\.nfe\.schema\.envEventoEConf\./com.telemetria.integration.nfe.schemas_eventos./g' \
        -e 's/br\.com\.swconsultoria\.nfe\.schema\.envEventoCancEConf\./com.telemetria.integration.nfe.schemas_eventos./g')

    # schema.envEventoInsucessoNFe, schema.envEventoCancInsucessoNFe → schemas_eventos
    modified=$(echo "$modified" | sed \
        -e 's/br\.com\.swconsultoria\.nfe\.schema\.envEventoInsucessoNFe\./com.telemetria.integration.nfe.schemas_eventos./g' \
        -e 's/br\.com\.swconsultoria\.nfe\.schema\.envEventoCancInsucessoNFe\./com.telemetria.integration.nfe.schemas_eventos./g')

    # schema.envEventoEntregaNFe, schema.envEventoCancEntregaNFe → schemas_eventos
    modified=$(echo "$modified" | sed \
        -e 's/br\.com\.swconsultoria\.nfe\.schema\.envEventoEntregaNFe\./com.telemetria.integration.nfe.schemas_eventos./g' \
        -e 's/br\.com\.swconsultoria\.nfe\.schema\.envEventoCancEntregaNFe\./com.telemetria.integration.nfe.schemas_eventos./g')

    # schema.resnfe.ResNFe / schema.retdistdfeint.* → schemas (nao schemas_eventos)
    modified=$(echo "$modified" | sed \
        -e 's/br\.com\.swconsultoria\.nfe\.schema\.resnfe\./com.telemetria.integration.nfe.schemas./g' \
        -e 's/br\.com\.swconsultoria\.nfe\.schema\.retdistdfeint\./com.telemetria.integration.nfe.schemas./g')
    ...
    modified=$(echo "$modified" | sed -E \
        -e 's/br\.com\.swconsultoria\.nfe\.schema\.evento([0-9]+)\.DetEvento\b/com.telemetria.integration.nfe.schemas_eventos.DetEvento\1/g')

    # qualquer schema.* restante → schemas_eventos (catch-all para outros eventos)
    modified=$(echo "$modified" | sed \
        -e 's/br\.com\.swconsultoria\.nfe\.schema\.[a-zA-Z0-9_]*\./com.telemetria.integration.nfe.schemas_eventos./g')
