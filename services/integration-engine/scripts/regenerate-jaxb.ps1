...
$BasePkg      = 'com.telemetria.integration.nfe'
...
if (Test-Path $pkgInfoPath) {
    ...
    if ($pkgInfoContent -notmatch [regex]::Escape($correctNs)) {
        $newPkgInfo = "@javax.xml.bind.annotation.XmlSchema(namespace = `"$correctNs`", elementFormDefault = javax.xml.bind.annotation.XmlNsForm.QUALIFIED)`npackage com.telemetria.integration.nfe.schemas_eventos;`n"
