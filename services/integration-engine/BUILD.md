...
| Package | Conteúdo |
|---|---|
| `com.telemetria.integration.nfe.schemas` | NFe v4.00 (TNFe, TInutNFe, TRetConsReciNFe, TRetConsStatServ, TConsCad, TRetConsCad, TResNFe, TRetDistDFeInt, TEndereco completo, TUf, TUfEmi, ...) |
| `com.telemetria.integration.nfe.schemas_eventos` | Todos os eventos SEFAZ (cancelamento, CCe, EPEC, manifestação, conciliação financeira, insucesso entrega, comprovante entrega, ator interessado, genérico) com classes renomeadas para evitar colisão |
...
A estratégia: rodar `xjc` em um **pacote temporário** (`com.telemetria.integration.nfe.tmp_evt_<código>`),
