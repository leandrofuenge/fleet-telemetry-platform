...
public class RotaService {
    ...
    /**
     * Calcula e armazena o trajeto usando OSRM. A rota permanece planejada até
     * esta operação terminar com sucesso; não existe aproximação em linha reta.
     */
    public Rota calcularComOsrm(Long id) {
        Rota rota = buscarPorId(id);
        if (rota.getLatitudeOrigem() == null || rota.getLongitudeOrigem() == null
                || rota.getLatitudeDestino() == null || rota.getLongitudeDestino() == null) {
            throw new BusinessException(ErrorCode.VALIDATION_ERROR,
                    "Coordenadas de origem e destino são obrigatórias para calcular a rota");
        }
        List<double[]> trajeto = osrmRoutingGateway.obterTrajeto(rota.getLatitudeOrigem(), rota.getLongitudeOrigem(),
                rota.getLatitudeDestino(), rota.getLongitudeDestino());
        if (trajeto.size() < 2) {
            throw new BusinessException(ErrorCode.VALIDATION_ERROR, "OSRM não retornou um trajeto utilizável");
        }
        java.util.ArrayList<PontoRota> pontos = new java.util.ArrayList<>();
        for (int i = 0; i < trajeto.size(); i++) {
            PontoRota ponto = new PontoRota(trajeto.get(i)[0], trajeto.get(i)[1], i + 1);
            ponto.setLatSnap(trajeto.get(i)[0]);
            ponto.setLngSnap(trajeto.get(i)[1]);
            pontos.add(ponto);
        }
        rota.setPontosRota(pontos);
        double distanciaKm = calcularDistanciaTotalRota(pontos);
        rota.setDistanciaPrevista(distanciaKm);
        rota.setTempoPrevisto(Math.max(1, (int) Math.ceil(distanciaKm / 60.0 * 60.0)));
        rota.setAtiva(false);
    ...
    private void validarCalculoOsrmRealizado(Rota rota) {
        ...
        if (pontosRota == null || pontosRota.size() < 2) {
            throw new BusinessException(ErrorCode.VALIDATION_ERROR,
                    "A rota precisa ter ao menos dois pontos calculados pelo OSRM antes de ser ativada");
        }
        
        // 2. Verificar se os pontos têm coordenadas de snap (lat_snap, lng_snap)
        boolean temSnap = pontosRota.stream()
                .allMatch(p -> p.getLatSnap() != null && p.getLngSnap() != null);
        
        if (!temSnap) {
            log.warn("Rota {} não possui pontos com snap-to-road OSRM", rota.getId());
            throw new BusinessException(ErrorCode.VALIDATION_ERROR,
                    "Todos os pontos da rota devem ser confirmados pelo OSRM antes da ativação");
        }
