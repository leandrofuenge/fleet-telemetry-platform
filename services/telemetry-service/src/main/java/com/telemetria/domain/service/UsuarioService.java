...
import com.telemetria.infrastructure.security.TotpService;
...
public class UsuarioService {
	...
	public Usuario criar(Usuario usuario, String senhaPlain) {
		...
		if (usuario.getPerfil() != Perfil.SUPER_ADMIN) {
			if (usuario.getTenantId() == null) {
				throw new BusinessException(ErrorCode.VALIDATION_ERROR, "Usuário deve pertencer a um tenant.");
			}
			tenantService.validarAcesso(usuario.getTenantId());
		}
	...
	private void validarHistoricoSenhas(Long usuarioId, String novaSenha) {
		List<String> ultimasSenhas = historicoSenhaRepository.findTop5ByUsuarioIdOrderByCriadoEmDesc(usuarioId)
				.stream().map(HistoricoSenha::getSenhaHash).toList();
	...
	@Transactional
	public com.telemetria.api.dto.response.MfaSetupResponse iniciarConfiguracaoMfa(String login, String senha) {
		Usuario usuario = autenticarParaMfa(login, senha);
		String segredo = totpService.gerarSegredo();
		usuario.setMfaSecret(segredo);
		usuario.setMfaAtivado(false);
		usuarioRepository.save(usuario);
		String emissor = "FrotaTelemetria";
		String conta = java.net.URLEncoder.encode(emissor + ":" + usuario.getLogin(), java.nio.charset.StandardCharsets.UTF_8);
		return new com.telemetria.api.dto.response.MfaSetupResponse(segredo,
				"otpauth://totp/" + conta + "?secret=" + segredo + "&issuer=" + emissor + "&algorithm=SHA1&digits=6&period=30");
	}

	@Transactional
	public void confirmarConfiguracaoMfa(String login, String senha, String codigo) {
		Usuario usuario = autenticarParaMfa(login, senha);
		if (!totpService.validarCodigo(usuario.getMfaSecret(), codigo)) {
			throw new BusinessException(ErrorCode.INVALID_CREDENTIALS, "Código MFA inválido.");
		}
		usuario.setMfaAtivado(true);
		usuarioRepository.save(usuario);
	}

	public void validarMfaParaLogin(Usuario usuario, String codigo) {
		boolean obrigatorio = isMfaObrigatorio(usuario);
		if (obrigatorio && !isMfaAtivado(usuario)) {
			throw new BusinessException(ErrorCode.FORBIDDEN, "MFA obrigatório ainda não foi configurado.");
		}
		if (isMfaAtivado(usuario) && !totpService.validarCodigo(usuario.getMfaSecret(), codigo)) {
			throw new BusinessException(ErrorCode.INVALID_CREDENTIALS, "Código MFA inválido.");
		}
	}

	public void validarAcessoDoUsuario(Usuario usuario) {
		if (usuario.getPerfil() != Perfil.SUPER_ADMIN && usuario.getTenantId() != null) {
			tenantService.validarAcesso(usuario.getTenantId());
		}
	}

	private Usuario autenticarParaMfa(String login, String senha) {
