...
public class MotoristaService {
    ...
    public Motorista salvar(Motorista motorista) {
        ...
        if (motorista.getTenantId() == null) {
            throw new BusinessException(ErrorCode.VALIDATION_ERROR, "Tenant é obrigatório para cadastrar motorista.");
        }
        tenantService.validarAcesso(motorista.getTenantId());

        motorista.setCpf(motorista.getCpf() == null ? null : motorista.getCpf().replaceAll("\\D", ""));
        ...
        boolean cpfDuplicado = isNovo
                ? motoristaRepository.existsByCpfAndTenantId(motorista.getCpf(), motorista.getTenantId())
                : motoristaRepository.existsByCpfAndTenantIdAndIdNot(motorista.getCpf(), motorista.getTenantId(), motorista.getId());
        if (cpfDuplicado) {
            throw new BusinessException(ErrorCode.MOTORISTA_DUPLICATE,
                    "Já existe um motorista com este CPF neste tenant.");
        }
