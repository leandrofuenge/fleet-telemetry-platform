    ...
    public void processarTelemetria(String mensagem, Acknowledgment ack) {
        ...
        try {
            ...
            try {
                ...
                // O resultado do snap é parte da telemetria e precisa sobreviver ao ciclo atual.
                if (saved.getLatSnap() != null && saved.getLngSnap() != null) {
                    telemetriaRepository.save(saved);
                }

                // RF12: regras configuráveis, cooldown e agrupamento antes das notificações.
                regraAlertaService.avaliar(saved);

                // RF28: colisão ou pânico abre sinistro e preserva a telemetria vinculada.
                if (Boolean.TRUE.equals(saved.getColisaoDetectada()) || Boolean.TRUE.equals(saved.getBotaoPanico())) {
                    operacaoService.abrirPorTelemetria(saved);
                }
                ...
                // RF14: a jornada é alimentada pela telemetria do motorista vinculado.
                if (saved.getMotoristaId() != null) {
                    boolean dirigindo = Boolean.TRUE.equals(saved.getIgnicao())
                            && saved.getVelocidade() != null && saved.getVelocidade() > 1.0;
                    jornadaService.registrarDirecao(saved.getTenantId(), saved.getMotoristaId(), saved.getVeiculoId(),
                            saved.getViagemId(), dirigindo, saved.getDataHora());
                }
