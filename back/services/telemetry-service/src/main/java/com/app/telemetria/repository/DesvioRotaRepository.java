...
public interface DesvioRotaRepository extends JpaRepository<DesvioRota, Long> {
    ...
    // Métodos adicionados para consulta por ID e resolução
    Optional<DesvioRota> findByRotaIdAndResolvidoFalse(Long rotaId);

    Optional<DesvioRota> findByVeiculoIdAndResolvidoFalse(Long veiculoId);
}