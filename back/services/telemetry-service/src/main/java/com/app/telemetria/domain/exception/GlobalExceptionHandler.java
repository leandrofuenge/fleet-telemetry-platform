...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleDisabledException(HttpServletRequest request) {
        ...
        return ResponseEntity.status(ErrorCode.ACCOUNT_DISABLED.getHttpStatus()).body(response);
    }

    // ================= CONTA BLOQUEADA (403) =================
    @ExceptionHandler(LockedException.class)
    public ResponseEntity<ErrorResponse> handleLockedException(HttpServletRequest request) {
        ...
        return ResponseEntity.status(ErrorCode.ACCOUNT_LOCKED.getHttpStatus()).body(response);
    }

    // ================= JWT (403) =================
    @ExceptionHandler(io.jsonwebtoken.JwtException.class)
    public ResponseEntity<ErrorResponse> handleJwtException(HttpServletRequest request) {
        ...
        return ResponseEntity.status(ErrorCode.TOKEN_INVALID.getHttpStatus()).body(response);
    }

    // ================= NOT FOUND (404) =================
    @ExceptionHandler({
        MotoristaNotFoundException.class,
        VeiculoNotFoundException.class,
        RotaNotFoundException.class,
    ...
    private ErrorCode determinarErrorCode(RuntimeException ex) {
        if (ex instanceof MotoristaNotFoundException) return ErrorCode.MOTORISTA_NOT_FOUND;
        if (ex instanceof VeiculoNotFoundException)  return ErrorCode.VEICULO_NOT_FOUND;
        if (ex instanceof RotaNotFoundException)     return ErrorCode.ROTA_NOT_FOUND;
        if (ex instanceof ViagemNotFoundException)   return ErrorCode.VIAGEM_NOT_FOUND;
        return ErrorCode.RESOURCE_NOT_FOUND;
    }

    // ================= CONFLITO (409) =================
    @ExceptionHandler({
        MotoristaDuplicateException.class,
        VeiculoDuplicateException.class,
        RotaDuplicateException.class
    })
    ...
    private ErrorCode determinarErrorCodeDuplicado(RuntimeException ex) {
        if (ex instanceof MotoristaDuplicateException) return ErrorCode.MOTORISTA_DUPLICATE;
        if (ex instanceof VeiculoDuplicateException)   return ErrorCode.VEICULO_DUPLICATE;
        if (ex instanceof RotaDuplicateException)      return ErrorCode.ROTA_DUPLICATE;
        return ErrorCode.DUPLICATE_RESOURCE;
    }

    // ================= REGRAS DE NEGÓCIO (422) =================
    @ExceptionHandler({
        RotaValidationException.class,
        VeiculoIndisponivelException.class,
        MotoristaIndisponivelException.class
    })
    ...
    private ErrorCode determinarErrorCodeBusiness(RuntimeException ex) {
        if (ex instanceof RotaValidationException)       return ErrorCode.ROTA_INVALID;
        if (ex instanceof VeiculoIndisponivelException)  return ErrorCode.VEICULO_INDISPONIVEL;
        if (ex instanceof MotoristaIndisponivelException)return ErrorCode.MOTORISTA_INDISPONIVEL;
        return ErrorCode.BUSINESS_ERROR;
    }

    // ================= INTEGRAÇÃO EXTERNA (424) =================
    @ExceptionHandler(WeatherApiException.class)
    public ResponseEntity<ErrorResponse> handleWeatherApiException(HttpServletRequest request) {
        ...
        return ResponseEntity.status(ErrorCode.WEATHER_API_ERROR.getHttpStatus()).body(response);
    }

    // ================= MUITAS REQUISIÇÕES (429) =================
    @ExceptionHandler(org.springframework.web.client.HttpClientErrorException.TooManyRequests.class)
    public ResponseEntity<ErrorResponse> handleTooManyRequestsException(HttpServletRequest request) {
        ...
        return ResponseEntity.status(ErrorCode.TOO_MANY_REQUESTS.getHttpStatus()).body(response);
    }
    ...
    public ResponseEntity<ErrorResponse> handleKafkaProducerException(HttpServletRequest request) {
        ...
        return ResponseEntity.status(ErrorCode.KAFKA_PRODUCER_ERROR.getHttpStatus()).body(response);
    }
    ...
    public ResponseEntity<ErrorResponse> handleMqttException(
        ...
        return ResponseEntity.status(ErrorCode.MQTT_CONNECTION_ERROR.getHttpStatus()).body(response);
    }
    ...
    public ResponseEntity<ErrorResponse> handleTimeoutException(HttpServletRequest request) {
        ...
        return ResponseEntity.status(ErrorCode.GATEWAY_TIMEOUT.getHttpStatus()).body(response);
    }
