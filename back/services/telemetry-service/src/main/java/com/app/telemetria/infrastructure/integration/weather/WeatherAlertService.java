package com.app.telemetria.infrastructure.integration.weather;
...
import com.app.telemetria.domain.enums.TipoAlerta; // IMPORT CORRETO
import com.app.telemetria.domain.exception.WeatherApiException;
...
public class WeatherAlertService {
    ...
    private enum CondicaoClimatica {
        CHUVA_FRACA(500, "Chuva fraca", "🌧️", SeveridadeAlerta.MEDIO),
        CHUVA_MODERADA(501, "Chuva moderada", "🌧️", SeveridadeAlerta.MEDIO),
        CHUVA_FORTE(502, "Chuva forte", "🌧️⚠️", SeveridadeAlerta.ALTO),
        CHUVA_MUITO_FORTE(503, "Chuva muito forte", "🌧️⚠️", SeveridadeAlerta.ALTO),
        CHUVA_EXTREMA(504, "Chuva extrema", "🌧️⛔", SeveridadeAlerta.ALTO),

        NEVE_FRACA(600, "Neve fraca", "❄️", SeveridadeAlerta.MEDIO),
        NEVE_MODERADA(601, "Neve moderada", "❄️", SeveridadeAlerta.MEDIO),
        NEVE_FORTE(602, "Neve forte", "❄️⚠️", SeveridadeAlerta.ALTO),

        TEMPESTADE(200, "Tempestade", "⛈️", SeveridadeAlerta.ALTO),
        NEBLINA(741, "Nevoeiro", "🌫️", SeveridadeAlerta.MEDIO),
        FUMAÇA(711, "Fumaça", "🔥", SeveridadeAlerta.MEDIO),
        POEIRA(761, "Poeira", "💨", SeveridadeAlerta.MEDIO);
    ...
    private enum FaixaTemperatura {
        MUITO_QUENTE(t -> t > 35, "🔥 Temperatura muito alta!", SeveridadeAlerta.ALTO),
        QUENTE(t -> t > 30, "🌡️ Temperatura elevada", SeveridadeAlerta.BAIXO),
        FRIO(t -> t < 5, "❄️ Temperatura baixa", SeveridadeAlerta.MEDIO),
        CONGELANTE(t -> t < 0, "⛔ Temperatura congelante!", SeveridadeAlerta.ALTO);
    ...
    private enum FaixaVento {
        VENTO_FORTE(v -> v > 50, "💨 VENTO FORTE! Segure firme!", SeveridadeAlerta.ALTO),
        VENTO_MODERADO(v -> v > 30, "💨 Vento moderado", SeveridadeAlerta.MEDIO);
    ...
    private SeveridadeAlerta determinarGravidade(WeatherResponse weather) {
        return Stream.of(
                ...
                Optional.ofNullable(weather.rain())
                        .map(r -> {
                            if (r._1h() > 10)
                                return SeveridadeAlerta.ALTO;
                            if (r._1h() > 5)
                                return SeveridadeAlerta.MEDIO;
                            return SeveridadeAlerta.BAIXO;
                        }))
    ...
    private void criarAlertaClimatico(Long veiculoId, Viagem viagem, String mensagem, SeveridadeAlerta gravidade) {
        ...
        Long tenantId = telemetriaRepository.findUltimaTelemetriaByVeiculoId(veiculoId)
                .map(t -> t.getTenantId())
                .orElse(1L);
        ...
        alerta.setTipo(TipoAlerta.CLIMA); // AGORA FUNCIONA
